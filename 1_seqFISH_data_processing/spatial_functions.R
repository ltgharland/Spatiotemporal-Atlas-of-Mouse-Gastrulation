...
updateMesenchymeLabel = function(mdt) {
    ...
    for (i in colsToChange) {
        ...
        # fac2 = factor(fac, levels = sort(c(levels(fac), newname)))
...
get_umap_graph <- function(umap_output) {
  # copied from karsten
  # umap_output is coming out from umap::umap() function
...
get_MVS_vals = function(predmat) {
  MVS_vals_svm = t(apply(predmat,1,function(x) {
    ...
    if (length(xx) == 0) {
      ...
      # stab = sort(table(x), decreasing = TRUE)
      # if (length(stab) == 1) return(0)
      # nodepath_dist[names(stab)[1],names(stab)[2]]
      # d = nodepath_dist[mode1,mode2]
      if (is.null(mode2)) {
        mode2 <- NA
      }
...
getMarkerArray = function(sce, 
  ...
  for (grouplevel in groups) {
    ...
    if (pseudobulk) {
      ...
      exprs <- assay(sce_pseudo, pseudoname)
      # print(names(assays(sce_pseudo)))
...
getJointPCA = function(atlas, spatial, 
  ...
  # if (inc_z) {
  #   batchFactor = factor(c(as.character(interaction(spatial$embryo, spatial$pos, spatial$z)),
  #                          as.character(colData(atlas)[,atlas_batch])))
  # } else {
  #   batchFactor = factor(c(as.character(interaction(spatial$embryo, spatial$pos)),
  #                          as.character(colData(atlas)[,atlas_batch])))
  # }
...
# function to predict within the joint PCA embedding
...
spatialPlotPointsGene = function(gene,
  ...
  # colour = ifelse(binary, gene > 0, log2(1+gene)))
...
processSeqFISH = function(params) {
  ...
  for (mRNA_file in mixedsort(mRNA_files)) {  
    ...
    # cell_info$x_global = (cell_info$Centroid_1 - range_x[1])*(bound_x[2] - bound_x[1])/(max(cell_info$Centroid_1 - range_x[1], na.rm = TRUE)) + bound_x[1]
    # cell_info$y_global = (cell_info$Centroid_2 - range_y[1])*(bound_y[2] - bound_y[1])/(max(cell_info$Centroid_2 - range_y[1], na.rm = TRUE)) + bound_y[1]
